...
# How many basis points BELOW the estimated stop-out to bid.
# In a yield auction, the stop-out is the HIGHEST yield still accepted.
# Bidding above it means exclusion; bidding below guarantees inclusion.
# The uniform-price rule means you receive the stop-out coupon regardless
# of how far below you bid — so you want to be just under, not way under.
# 50 bps of safety margin below the estimated stop-out is a good starting point.
SNIPE_BUFFER_YIELD = 0.0050  # 50 bps BELOW estimated clearing yield
...
def compute_bid_yield(estimated_stop_out: float) -> float:
    ...
    The stop-out is the HIGHEST yield accepted in the auction.
    Bidding above it → excluded. Bidding below it → included.
    Under the uniform-price rule we receive the stop-out coupon rate
    regardless of our bid, so we want to be just under — not way under —
    to avoid unnecessarily signalling willingness to accept a low coupon
    if our estimate turns out to be too pessimistic.
    """
    bid = estimated_stop_out - SNIPE_BUFFER_YIELD
    return max(bid, 0.0001)  # floor at 1 bp to avoid zero/negative bids
